...
public:
	...
	void Log(const string& file, const string& line, const string& message) {
		if (log_file == true && log_line == true) {
			os << file << ":" << line << " " << message << endl;
		}
		else if (log_file == false && log_line == true) {
			os << "Line " << line << " " << message << endl;
		}
		else if (log_file == true && log_line == false) {
			os << file << " " << message << endl;
		}
		else if (log_file == false && log_line == false) {
			os << message << endl;
		}
	}
private:
...
#define LOG(logger, message) {							\
	...
	os_line << __LINE__;								\
	logger.Log(os_file.str(),os_line.str(),message);	\
}
